...
#if defined(__AVR_ATtiny24A__) | defined(__AVR_ATtiny24__) | \
	defined(__AVR_ATtiny44A__) | defined(__AVR_ATtiny44__) | \
	defined(__AVR_ATtiny84A__) | defined(__AVR_ATtiny84__)
#define DDR_USI DDRA
...
#define tempUSISR_8bit ((1 << USISIF) | (1 << USIOIF) | (1 << USIPF) | (1 << USIDC) | (0x0 << USICNT0)) // Prepare register value to: Clear flags, and
																										// set USI to shift 8 bits i.e. count 16 clock edges.
#define tempUSISR_1bit ((1 << USISIF) | (1 << USIOIF) | (1 << USIPF) | (1 << USIDC) | (0xE << USICNT0)) // Prepare register value to: Clear flags, and
...
uint8_t USI_TWI_Start(uint8_t addr);
uint8_t USI_TWI_Stop(void);
uint8_t USI_TWI_Write(uint8_t *data, uint8_t size);
uint8_t USI_TWI_Read(uint8_t *data, uint8_t size);
